...
def main():
    global root, canvas, gun, screen1, target
    root = tk.Tk()
...
class Ball():
    ...
    def move(self):
        ...
        Метод описывает перемещение мяча за один кадр перерисовки. То есть, обновляет значение
    ...
    def hittest(self, obj):
        """Функция проверяет стракивается ли данный объект с целью, описываемой в объекте obj.
        Args:
...
class Gun():
    def __init__(self):
        ...
        self.x = 20
        self.y = 450
        self.id = canvas.create_line(self.x, self.y, 50, 420, width=7) #FIXME: don't know how?
...
class Target():
    ...
    def hit(self, points=1):
        ...
        self.points += points
        canvas.itemconfig(self.id.points, text=self.points)
